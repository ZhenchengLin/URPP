...
from app.services.course_knowledge.course_pack_v01 import (
    CoursePackV01, resolve_course_knowledge_v01,
)
...
class LocalLearningWorkspaceV01:
    ...
    def inspect_course_pack(self, *, pack_sha256: str, session_id: str) -> dict:
        """Read only the approved, student-visible sources of a pinned Session.

        An uploaded Objective is supplied by the user, not generated by the
        model. This view does not invoke a gateway or modify chat history.
        """
        snapshot = self.resume(pack_sha256=pack_sha256, session_id=session_id)
        pack = self._load_pack(pack_sha256)
        context = resolve_course_knowledge_v01(
            pack=pack,
            course_id=snapshot.course_id,
            objective_id=snapshot.objective_id,
            expected_pack_revision=snapshot.pack_revision,
        )
        sources = []
        for source in context.sources:
            locator = urlsplit(source.source_locator)
            pages = parse_qs(locator.query).get("pages", [None])[0]
            sources.append({
                "source_id": source.source_id,
                "filename": unquote(locator.netloc),
                "pages": pages,
        ...
        return {
            ...
            "sources": sources,
            "model_generated_course_path": False,
        }
